...
| **LAC** | Standard Softmax classifier | Pooled exact split-conformal score $1-p_Y$ | None | No | Produces probability superlevel sets and may be empty or fragmented in ordinal order. |
| **APS** | Standard Softmax classifier | Pooled exact calibration of cumulative mass through the true label | Smallest stable probability-ranked prefix reaching the calibrated threshold | No | Adapts cardinality to probability concentration, but ignores ordinal adjacency and may fragment. |
| **OAPS** | Standard Softmax classifier | Pooled exact calibration of greedy-set entry thresholds | Greedy expansion from the modal class toward the more probable adjacent class | No | Implements Lu et al. Algorithm 1 and targets marginal coverage with nested, contiguous sets. |
...
| **COPOC** | Non-parametric Eq. (5) unimodal classifier | Pooled APS cumulative-mass score | Smallest stable probability-ranked prefix reaching the calibrated threshold | No | Uses the paper's unimodal architecture with APS; contiguity follows from the verified unimodal probabilities and stable ordering. |

LAC, APS, OAPS, min-CPS, min-RCPS, and risk control use a standard Softmax classification backbone. COPOC uses the non-parametric unimodal Eq. (5) backbone; the legacy binomial-LAC surrogate is not canonical COPOC. OCQR uses the quantile-regression backbone with either a physical numeric target or a documented ordinal index embedding. Using one checkpoint architecture for every method is not a valid comparison.
