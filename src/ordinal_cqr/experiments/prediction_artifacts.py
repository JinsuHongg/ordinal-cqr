...
def evaluate(rows:list[dict[str,Any]], k:int, alpha:float, ocqr:bool=False)->dict[str,Any]:
 ...
 def summary(sets):
  sizes=[len(s) for s in sets]; comps=[_components(s) for s in sets]; return {"mean_set_size":sum(sizes)/len(sizes),"median_set_size":median(sizes),"mean_ordinal_span":sum((s[-1]-s[0]+1) if s else 0 for s in sets)/len(sizes),"singleton_rate":sum(x==1 for x in sizes)/len(sizes),"full_set_rate":sum(x==k for x in sizes)/len(sizes),"contiguous_set_rate":sum(x<=1 for x in comps)/len(sizes),"fragmented_set_rate":sum(x>1 for x in comps)/len(sizes),"mean_connected_components":sum(comps)/len(comps)}
 ...
 out={"aggregate":{"marginal_coverage":coverage(final),**summary(final),"macro_class_coverage":sum(pc.values())/len(pc),"worst_class_coverage":min(pc.values()),"worst_undercoverage":min(gaps.values()),"mean_absolute_class_coverage_deviation":sum(abs(v) for v in gaps.values())/len(gaps),"classes_below_target":sum(v<0 for v in gaps.values()),"classes_more_than_002_below_target":sum(v<-0.02 for v in gaps.values())},"per_class":[{"class_id":int(c),"count":counts[c],"coverage":pc[c],"coverage_gap":gaps[c],"mean_set_size":sum(len(r["prediction_set_final"]) for r in by[int(c)])/counts[c]} for c in sorted(pc,key=int)]}
 if ocqr:
  fallback=[list(range(k)) if not s else s for s in raw]; out["aggregate"].update({"raw_marginal_coverage":coverage(raw),"final_marginal_coverage":coverage(final),"raw_mean_set_size":sum(map(len,raw))/len(rows),"final_mean_set_size":sum(map(len,final))/len(rows),"raw_empty_rate":sum(not s for s in raw)/len(rows),"fragmented_raw_set_rate":sum(_components(s)>1 for s in raw)/len(rows),"fallback_activation_rate":sum(not s for s in raw)/len(rows),"hull_activation_rate":sum(len(f)!=len(b) for f,b in zip(final,fallback))/len(rows),"fallback_inflation":sum(len(f)-len(s) for f,s in zip(fallback,raw))/len(rows),"hull_inflation":sum(len(f)-len(b) for f,b in zip(final,fallback))/len(rows),"total_inflation":sum(len(f)-len(s) for f,s in zip(final,raw))/len(rows)})
 return out
