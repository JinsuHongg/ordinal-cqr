...
def main():
 ...
 spec=DATASETS[dataset];manifest=spec['manifest'];data=manifest.read_bytes();manifest_hash=spec['hash'];thresholds=spec['thresholds']
 ...
 out=Path('outputs/conference_v0_3')/dataset/z.method/f'seed_{z.seed}'
 if out.exists() and not z.overwrite: raise RuntimeError(f'{out} exists; use --overwrite')
 out.mkdir(parents=True,exist_ok=True);logger=setup_logger(out);global _ACTIVE_RUN_DIR,_ACTIVE_LOGGER;_ACTIVE_RUN_DIR,_ACTIVE_LOGGER=out,logger;logger.info('run start output=%s dataset=%s method=%s seed=%s',out,dataset,z.method,z.seed);write_run_status(out,'started',current_stage);cfg['seed']=z.seed;(out/'config.yaml').write_text(yaml.safe_dump(cfg,sort_keys=False));current_stage='building_data';write_run_status(out,'started',current_stage);logger.info('stage %s',current_stage)
